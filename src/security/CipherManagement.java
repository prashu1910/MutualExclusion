...
public class CipherManagement {
    ...
    public static String encrypt(String message, PublicKey publicKey)
    {
        try
        {
           ...
           // Util.println("Encryption "+cipherText.length + " :: " +Arrays.toString(cipherText));
            String msg = Base64.getEncoder().encodeToString(cipherText);
            //Util.println("cipherText = encryption" + msg.length() + " :: "+msg );   
    ...
    public static String decrypt(String cipherText, PrivateKey privateKey) 
    {
        try
        {
           // Util.println("decryption msg " + cipherText.length() + " :: "+cipherText);
            byte[] bytes = Base64.getDecoder().decode(cipherText.trim());
         //   Util.println("Decryption  "+ bytes.length + " :: " + Arrays.toString(bytes));
